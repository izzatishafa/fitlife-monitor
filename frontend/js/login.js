...
function showLoginTab(tab) {
  ...
  if (tab === "login") {
    ...
    loginTab.className =
      "flex-1 pb-2 text-center font-semibold border-b-2 border-indigo-900 text-indigo-900 transition";
    registerTab.className =
      "flex-1 pb-2 text-center font-semibold border-b-2 border-transparent text-gray-400 hover:text-gray-600 transition";
  } else {
...
async function handleLogin() {
  ...
  try {
    ...
    if (username === "demo" && password === "demo123") {
      ...
      showAlert(
        "Backend server not available. Use demo mode or try: demo/demo123",
        "error"
      );
    }
  }
...
async function handleRegister() {
  ...
  try {
    ...
    showAlert(
      "Backend server not available. Registration requires server connection.",
      "error"
    );
  }
}
